...
`),chart={},bpm=1,anger=1;for(let line of lines){let[key,...rest]=line.split(":");if(!key||rest.length===0)continue;let rawValue=rest.join(":").trim(),trimmedKey=key.trim(),parsedValue=rawValue;try{parsedValue=JSON.parse(rawValue)}catch{rawValue.startsWith("[")&&rawValue.endsWith("]")&&(parsedValue=rawValue.slice(1,-1).split(",").map(s=>s.trim().replace(/^["'](.*)["']$/,"$1")))}trimmedKey==="bpm"?bpm=parseFloat(rawValue):trimmedKey==="anger"?anger=parseFloat(rawValue):chart[trimmedKey]=parsedValue}return{bpm,anger,chart}}__name(parseRaceChartBlock,"parseRaceChartBlock");function generateChartId(){return"race-chart-"+Math.random().toString(36).slice(2,10)}__name(generateChartId,"generateChartId");var chartId=generateChartId(),bpId=`bp-toggle-${chartId}`,angerId=`anger-toggle-${chartId}`,RaceChartBlockPlugin=__name(()=>({name:"RaceChartBlockPlugin",markdownPlugins(){return[()=>tree=>{visit8(tree,"code",(node,index,parent)=>{if(node.lang!=="RaceChart"||typeof index!="number"||!parent)return;let{bpm,anger,chart}=parseRaceChartBlock(node.value),chartJson=JSON.stringify(chart),configJson=JSON.stringify({bpm,anger}),html=`
<canvas
  class="chart-block"
  id="${chartId}"
